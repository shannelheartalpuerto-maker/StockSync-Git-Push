    ...
    <table class="table table-hover align-middle">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><span class="badge bg-info text-dark">+{{ $log->quantity }}</span></td>
                <td>{{ $log->user->name ?? 'System' }}</td>
                <td>
                    <div class="d-flex align-items-center">
                        <span class="text-truncate" style="max-width: 200px;">{{ $log->notes }}</span>
                        <button type="button" class="btn btn-sm btn-link text-decoration-none ms-2" data-bs-toggle="modal" data-bs-target="#returnDetailsModal{{ $log->id }}">
            ...
            @empty
            <tr>
                <td colspan="5" class="text-center text-muted py-4">No returned records found.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
</div>
{{ $returnedStocks->appends(['in_page' => $stockIns->currentPage(), 'out_page' => $stockOuts->currentPage(), 'damaged_page' => $damagedStocks->currentPage(), 'period' => $period])->links() }}

<!-- Return Details Modals -->
@foreach($returnedStocks as $log)
@php
            ...
            <div class="modal-body p-4">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <div><span class="badge bg-info text-dark">+{{ $log->quantity }}</span></div>
                    </div>
                    <div class="col-12">
